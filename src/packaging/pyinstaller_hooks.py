...
提供PyInstaller打包时需要的隐藏导入和钩子目录，适配新的配置架构
"""
...
def get_data_files() -> List[tuple]:
    ...
    config_template_paths = [
        # 如果存在config目录的模板文件
        project_root / "config" / "config.template.json",
        # 也包含在打包资源中创建的模板
        project_root / "src" / "config" / "templates" / "config.template.json"
    ]
    
    for template_path in config_template_paths:
        if template_path.exists():
            data_files.append((str(template_path), "config.template.json"))
            break
    ...
    # 数据库配置文件
    db_config_dir = project_root / "src" / "database" / "config"
    if db_config_dir.exists():
        for config_file in db_config_dir.glob("*.py"):
            if config_file.name != "__init__.py":
                data_files.append((str(config_file), f"database/config/{config_file.name}"))
