...
class JsonConfigLoader:
    ...
    def apply_json_to_config(self, config: T, json_data: Dict[str, Any]) -> T:
        ...
        if isinstance(config, AppConfig):
            return cast(T, self._apply_to_app_config(config, json_data))
        elif isinstance(config, LoggerConfig):
            return cast(T, self._apply_to_logger_config(config, json_data))
        elif isinstance(config, DatabaseConfig):
            return cast(T, self._apply_to_database_config(config, json_data))
        else:
            logger.warning(f"不支持的配置类型: {type(config)}")
            return config
    ...
    def _apply_to_app_config(self, config: AppConfig, json_data: Dict[str, Any]) -> AppConfig:
        ...
        # 直接映射的字段
        direct_fields = [
            'repo_path', 'search_term', 'is_regex', 'enable_validate', 'validate_workers'
        ]
    ...
    def _apply_to_logger_config(self, config: LoggerConfig, json_data: Dict[str, Any]) -> LoggerConfig:
        ...
        if 'logging' in json_data:
            ...
            if 'file_path' in logging_config:
                config.log_file_path = logging_config['file_path']
            if 'rotation' in logging_config:
                config.log_rotation = logging_config['rotation']
            if 'retention' in logging_config:
                config.log_retention = logging_config['retention']
    ...
    def _apply_to_database_config(self, config: DatabaseConfig, json_data: Dict[str, Any]) -> DatabaseConfig:
        ...
        if 'database' in json_data:
            ...
            if 'pool_size' in db_config:
                config.connection_pool_size = db_config['pool_size']
            if 'max_overflow' in db_config:
                # 注意：DatabaseConfig中没有max_overflow属性，忽略
                pass
            if 'pool_timeout' in db_config:
                config.connection_timeout = db_config['pool_timeout']
