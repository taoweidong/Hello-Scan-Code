...
def main():
    ...
    # 创建spec文件
    spec_file = helper.create_spec_file(platform="windows", onefile=True)
    ...
    exe_file = dist_dir / "hello-scan-code.exe"
    
    if not exe_file.exists():
        logger.error("未找到构建的可执行文件")
        sys.exit(1)
    
    # 创建发布目录
    release_dir = project_root / "dist" / "hello-scan-code-v1.0.0-windows"
    release_dir.mkdir(exist_ok=True)
    
    # 复制可执行文件
    shutil.copy2(exe_file, release_dir / "hello-scan-code.exe")
    
    # 复制资源文件
    helper.copy_resources_to_dist(str(release_dir))
    
    # 创建打包信息
    helper.create_package_info(str(release_dir), "windows")
    
    # 创建使用说明
    create_usage_instructions(release_dir)
    
    logger.info(f"Windows构建完成，输出目录: {release_dir}")


def create_usage_instructions(release_dir: Path):
    """创建使用说明文件"""
    usage_content = """# Hello-Scan-Code 使用说明
